...
class MCPRegistry:
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            ...
            # isError=True means the server raised (e.g. pydantic ValidationError on wrong params)
            if getattr(result, "is_error", False):
                ...
                for block in (result.content if hasattr(result, "content") else []):
                    if hasattr(block, "text"):
                        ...
                        parts.append(block)
                error_detail = " ".join(parts) if parts else "tool execution error"
                return json.dumps({"success": False, "message": error_detail})
            parts = []
