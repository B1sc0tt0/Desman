...
async def api_models():
    ...
    curated = [
        {"display_name": name, "ollama_model": model, "curated": True, "type": "text"}
    ...
    try:
        ...
        extra = [
            {"display_name": m, "ollama_model": m, "curated": False, "type": "text"}
    ...
    ext_models = [
        {"display_name": m["display_name"], "ollama_model": m["model_string"],
         "curated": True, "provider": m["provider"], "type": m.get("type", "text")}
...
def _resolve_model(display_name: str) -> dict | None:
    """Return {model_string, type, provider} for a display_name, or None if unknown."""
    if _model_map and display_name in _model_map:
        return {"model_string": _model_map[display_name], "type": "text", "provider": None}
    ...
    for m in get_external_models(load_external_apis()):
        if m["display_name"] == display_name:
            if m.get("type") == "image":
                # Strip the synthetic "hfimage:" prefix to get the bare HF model id
                hf_model = m["model_string"].removeprefix("hfimage:")
                return {"model_string": hf_model, "type": "image", "provider": "huggingface"}
            return {"model_string": m["model_string"], "type": m.get("type", "text"), "provider": m.get("provider")}
    return None
...
async def _image_generator(prompt: str, hf_model: str) -> AsyncGenerator[str, None]:
    """Call HuggingFace image generation API and stream image_generating / image_done events."""
    import base64
    import httpx
    ...
    if not api_key:
        yield _sse("error", {"message": "HuggingFace API key not configured. Add it in Settings → External APIs."})
        return

    yield _sse("image_generating", {"model": hf_model, "prompt": prompt})
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            r = await client.post(
                ...
                headers=headers,
                json={"model": hf_model, "prompt": prompt, "n": 1},
            )

            if r.status_code == 404:
                # Fallback to HF router direct model path
                headers_fb = {**headers, "Content-Type": "application/json"}
                r = await client.post(
                ...
                if r.status_code != 200:
                    yield _sse("error", {"message": f"Image generation failed (HTTP {r.status_code}): {r.text[:300]}"})
                    return
                ...
                # Router endpoint returns OpenAI-compatible JSON
                data = r.json()
                item = data["data"][0]
                if item.get("b64_json"):
                    ...
                    mime_type = img_r.headers.get("content-type", "image/jpeg").split(";")[0].strip()
                    b64 = base64.b64encode(image_bytes).decode()
                else:
                    yield _sse("error", {"message": "Image generation returned no image data."})
                    return

        yield _sse("image_done", {
            "image_b64": b64,
            "mime_type": mime_type,
            "model": hf_model,
            "prompt": prompt,
        ...
        yield _sse("error", {"message": f"Image generation error: {str(e)}"})
...
# ── Image generation (first-class endpoint) ───────────────────────────────────
...
async def api_image_generate(body: ImageGenerateBody):
    """SSE stream: image_generating → image_done (or error)."""
    resolved = _resolve_model(body.model)
    if not resolved:
        async def _err():
            yield _sse("error", {"message": f"Unknown model: {body.model}"})
    ...
    if resolved["type"] != "image":
        async def _err():
            yield _sse("error", {"message": f"'{body.model}' is a text model, not an image model."})
        return StreamingResponse(_err(), media_type="text/event-stream",
                                 headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"})
    return StreamingResponse(
        _image_generator(body.prompt, resolved["model_string"]),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )
...
class CSVRunBody(BaseModel):
    ...
    filename: str = ""
    disabled_servers: list[str] = []
    # "freshdesk_tickets" | "freshdesk_ticket_fields" | "freshservice_assets"
...
# Patterns that indicate the model narrated a tool call in prose instead of invoking it.
# Only checked when no real tool_call event was emitted for a row.
_PHANTOM_PATTERNS = [
    # Model names a server-prefixed tool (freshdesk_create_ticket, freshservice_*, rag_*)
    r'\b(?:freshdesk|freshservice|rag)_[a-z_]+',
    # Model writes JSON that looks like a function/tool call
    r'"(?:tool|function|name)"\s*:\s*"[a-z_]+"',
    # Model claims success / reports an ID as if a ticket or article was created
    r'(?:erfolgreich\s+erstellt|successfully\s+created|ticket[- #]+\d+|artikel[- ]?id\s*[:|]\s*\d+|article[- ]?id\s*[:|]\s*\d+)',
]
_PHANTOM_RE = re.compile("|".join(_PHANTOM_PATTERNS), re.IGNORECASE)


def _is_phantom_tool_call(text: str) -> bool:
    """Return True if model output suggests it narrated a tool call without invoking one."""
...
def _build_row_prompt(import_type: str, row: dict, row_num: int, total: int) -> str:
    ...
    elif import_type == "freshdesk_ticket_fields":
        return (
            f"You are importing row {row_num} of {total} into Freshdesk as a custom ticket field.\n"
            f"Call create_ticket_field ONCE using EXACTLY the field values provided below.\n"
            f"Rules:\n"
            f"- Do NOT translate, rename, or modify any values.\n"
            f"- The 'label' column is the label, 'field_type' is the type, 'choices' is comma-separated options (only for custom_dropdown).\n"
            f"- Do NOT call list_ticket_fields or any other tool.\n"
            f"- Stop immediately after create_ticket_field responds — do not verify or follow up.\n\n"
            f"{data_str}"
        )
...
async def _csv_bulk_generator(body: CSVRunBody) -> AsyncGenerator[str, None]:
    ...
    if body.import_type in ("freshdesk_tickets", "freshdesk_ticket_fields") and "freshservice" not in auto_disabled:
            ...
            if chunk.startswith("event: tool_result\n"):
                try:
                    ...
                    if '"success": false' in chunk.lower():
                        tool_error = True
            # Accumulate model prose to detect phantom tool calls later
            if chunk.startswith("event: text\n"):
                try:
                    data_line = chunk.split("\n", 2)[1].removeprefix("data: ")
                    response_text += json.loads(data_line).get("chunk", "")
                except Exception:
                    pass
            yield chunk

        # Phantom tool call: model narrated success/tool-name in prose but never invoked
        phantom_tool_call = (not tool_called) and _is_phantom_tool_call(response_text)
...
RAG_DATA_DIR = Path(__file__).parent.parent / "mcp_servers" / "rag" / "data"
_ALLOWED_RAG_SUFFIXES = {".pdf", ".docx", ".txt", ".md"}
...
async def api_rag_upload(
    ...
    """
    Save an uploaded file to mcp_servers/rag/data/ and ingest it into RAG.

    The collection and optional doc_id can be passed as query params:
      POST /api/rag/upload?collection=customers&doc_id=acme_profile
    """
    if not _session_manager:
        raise HTTPException(status_code=503, detail="Session manager not ready")

    suffix = Path(file.filename).suffix.lower()
    if suffix not in _ALLOWED_RAG_SUFFIXES:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported file type '{suffix}'. Allowed: {', '.join(_ALLOWED_RAG_SUFFIXES)}",
        )

    RAG_DATA_DIR.mkdir(parents=True, exist_ok=True)
    safe_name = Path(file.filename).name.replace("..", "").replace("/", "_").replace("\\", "_")
    dest = RAG_DATA_DIR / safe_name
    content = await file.read()
    dest.write_bytes(content)

    args = {"file_path": str(dest), "collection": collection}
    if doc_id.strip():
        args["doc_id"] = doc_id.strip()

    result_raw = _session_manager.call_tool("rag_ingest_document", args)
    try:
...
async def api_rag_collections():
    """List all RAG collections with chunk counts."""
    if not _session_manager:
        raise HTTPException(status_code=503, detail="Session manager not ready")
    result_raw = _session_manager.call_tool("rag_list_collections", {})
    try:
...
@app.get("/api/rag/documents/{collection}")
async def api_rag_documents(collection: str):
    """List all documents in a RAG collection."""
    if not _session_manager:
        raise HTTPException(status_code=503, detail="Session manager not ready")
    result_raw = _session_manager.call_tool("rag_list_documents", {"collection": collection})
    try:
        return json.loads(result_raw)
    except Exception:
        return {"raw": result_raw}
...
@app.delete("/api/rag/documents/{collection}/{doc_id}")
async def api_rag_delete_document(collection: str, doc_id: str):
    """Remove a single document from a RAG collection."""
    if not _session_manager:
        raise HTTPException(status_code=503, detail="Session manager not ready")
    result_raw = _session_manager.call_tool("rag_delete_document", {"collection": collection, "doc_id": doc_id})
    try:
        return json.loads(result_raw)
    except Exception:
        return {"raw": result_raw}
