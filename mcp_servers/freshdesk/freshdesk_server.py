...
@mcp.tool()
def create_ticket_field(label: str, field_type: str, choices: str | None = None) -> dict:
    """Creates a new custom ticket field. For dropdowns, provide choices as comma-separated values.
    After creating, always tell the user that the field is hidden from customers by default and
    is only visible to agents. No further action is needed after creation.

    Args:
        label: Display label e.g. "Customer Region"
        field_type: Field type e.g. "custom_text", "custom_dropdown", "custom_checkbox", "custom_date", "custom_number"
        choices: Comma-separated options for dropdowns e.g. "North,South,East,West" (only for custom_dropdown)
    """
    valid_types = ['custom_text', 'custom_dropdown', 'custom_checkbox', 'custom_date', 'custom_number']
    if field_type not in valid_types:
        return {'success': False, 'message': f'Invalid field_type "{field_type}". Valid types: {valid_types}'}
    if field_type == 'custom_dropdown' and not choices:
        return {'success': False, 'message': 'choices is required for custom_dropdown e.g. "North,South,East,West"'}
    payload: dict = {'label': label, 'label_for_customers': label, 'type': field_type}
    if choices:
    ...
    with get_client() as client:
        ...
        if not resp.is_success:
            return {'success': False, 'message': f'API error {resp.status_code}: {resp.text}'}
        f = resp.json()
        return {'success': True, 'field_id': f.get('id'), 'message': f'Ticket field "{label}" created successfully.'}
